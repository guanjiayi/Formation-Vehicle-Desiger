...
double Follower_yaw_rate=0;       // 跟随车横摆角速度
double Follower_steer_angle=0;    // 跟随车方向盘转角
...
// 规划线程标志位初始化
bool PlanningThflag=false; 

// 前车轨迹初始化
vector<TrajInfo>LeaderTraj;


// 互斥锁
mutex control_mut;             // 控制线程互斥锁
mutex msg_mut;                 // CAN通信线程互斥锁
mutex PlanningLock;            // 规划线程互斥锁

int main()
{
    CANMessage CANUwbPos;           // 数据接收对象
    CANMessage CANLeaderSta;        // 数据接收对象
    CANMessage CANCurVeh;           // 数据接收对象
    CANMessage CANYawVeh;           // 数据接收对象
    CANMessage CANSteerVeh;      // 本车方向盘转角数据接收对象
    CANMessage CANSendControl;      // 数据发送对象
   ...
   // 接收UWB位置信息线程
    ...
    thread RecivePosTh(bind(&CANMessage::CAN_receive,&CANUwbPos,UWB_POSITION_MSG));       //receive UWB position (args:ID CANchannel)
    // 接收领航车状态信息线程
    // thread ReciveLeaTh(&DataTransporter.CAN_receive,UWB_LEADERSTATE_MSG);    //receive UWB leader_state
    thread ReciveLeaTh(bind(&CANMessage::CAN_receive,&CANLeaderSta,UWB_LEADERSTATE_MSG));    //receive UWB leader_state
    // 接收车辆ACC信息线程
    // thread ReciveAccTh(&DataTransporter.CAN_receive,VEHICLE_ACC_MSG);        //receive follower ACC
    thread ReciveAccTh(bind(&CANMessage::CAN_receive,&CANCurVeh,VEHICLE_ACC_MSG));        //receive follower ACC
    
    //创建接收跟随车横摆角速度线程
    thread ReciveYawTh(bind(&CANMessage::CAN_receive,&CANYawVeh,VEHICLE_RATE_MSG));
    
    //创建接收跟随车当前方向盘转角线程
    thread ReciveSteerTh(bind(&CANMessage::CAN_receive,&CANSteerVeh,VEHICLE_STEER_MSG));
    // 开启log数据记录线程
    //thread LogTh(&Logger.LogUpdate);
    usleep(SAMPLE_TIME);
    thread LogTh(std::bind(&Log::LogUpdate,&Logger));
    ...
    thread PlanningTh(std::bind(&Planning::UpdatePlanning,&Planner));
    // 控制线程
    // thread ControlTh(&Controler.UpdateControl);                             //control signal update
    thread ControlTh(std::bind(&Control::UpdateControl,&Controler));                             //control signal update
    // 数据收发线程
    thread SendDataTh(bind(&CANMessage::CAN0_update,&CANSendControl));                         //send control signals
